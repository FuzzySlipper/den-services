SERVICES := shared delivery runtime observation gateway conversation timeline mcp visual-contract visual-inspect doc-publish artifacts migration integration librarian handoff playwright-broker devserver-broker den-serve
...
ifndef DEN_MCP_SMOKE_MESSAGES_URL
	$(error DEN_MCP_SMOKE_MESSAGES_URL is required, for example http://127.0.0.1:8093 when running on den-srv)
endif
...
ifndef DEN_MCP_SMOKE_REVIEW_URL
	$(error DEN_MCP_SMOKE_REVIEW_URL is required, for example http://127.0.0.1:8096 when running on den-srv)
endif
