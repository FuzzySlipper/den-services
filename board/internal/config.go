...
func (f configFile) toConfig(values sharedconfig.Values) (*Config, error) {
	projectTimeout, err := parseRequiredDuration("projects_request_timeout", f.ProjectsRequestTimeout)
	if err != nil {
		return nil, err
	}
...
func (c *Config) Validate() error {
	...
	if c.ProjectsRequestTimeout <= 0 {
		return errors.New("projects_request_timeout must be positive")
	}
...
func configPath() string {
	path := os.Getenv("BOARD_CONFIG_PATH")
	if path == "" {
