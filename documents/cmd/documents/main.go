...
import (
	...
	documents "den-services/documents/internal"
...
func main() {
	if len(os.Args) == 2 && os.Args[1] == "--version" {
		fmt.Printf("documents %s %s %s\n", version, commit, builtAt)
		return
	}
	cfg, err := documents.LoadConfig()
	if err != nil {
	...
	service := documents.NewService(
		documents.NewStore(pool),
		documents.NewProjectScopeClient(cfg.ProjectsBaseURL, cfg.ProjectsToken),
		documents.NewAgentGuidanceClient(cfg.AgentGuidanceBaseURL, cfg.AgentGuidanceToken),
		time.Now,
	)
	server, err := documents.NewHTTPServer(cfg, info, service)
	if err != nil {
		slog.Error("building server", "error", err)
		os.Exit(1)
	}
	slog.Info("documents listening", "bind_addr", cfg.BindAddr)
	if err := server.ListenAndServe(); err != nil {
		slog.Error("documents server", "error", err)
		os.Exit(1)
	}
}
...
func buildInfo() (health.BuildInfo, error) {
	...
	return health.NewBuildInfo("documents", version, commit, parsedBuiltAt)
}
