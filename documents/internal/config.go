...
type Config struct {
	...
	AgentGuidanceBaseURL string
	AgentGuidanceToken   string
	HTTP                 HTTPConfig
}
...
type configFile struct {
	...
	AgentGuidanceBaseURLEnv string         `yaml:"agent_guidance_base_url_env"`
	AgentGuidanceTokenEnv   string         `yaml:"agent_guidance_token_env"`
...
func LoadConfigFromPathWithValues(path string, values sharedconfig.Values) (*Config, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("reading documents config %s: %w", path, err)
	}
	var file configFile
	if err := yaml.Unmarshal(data, &file); err != nil {
		return nil, fmt.Errorf("parsing documents config %s: %w", path, err)
	}
...
func (f configFile) toConfig(values sharedconfig.Values) (*Config, error) {
	...
	return &Config{
		...
		AgentGuidanceBaseURL: values.String(f.AgentGuidanceBaseURLEnv, ""),
		AgentGuidanceToken:   values.String(f.AgentGuidanceTokenEnv, serviceToken),
...
func configPath() string {
	path := os.Getenv("DOCUMENTS_CONFIG_PATH")
	if path == "" {
