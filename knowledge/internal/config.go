...
func LoadConfigFromPathWithValues(path string, values sharedconfig.Values) (*Config, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("reading knowledge config %s: %w", path, err)
	}
	var file configFile
	if err := yaml.Unmarshal(data, &file); err != nil {
		return nil, fmt.Errorf("parsing knowledge config %s: %w", path, err)
	}
...
func configPath() string {
	path := os.Getenv("KNOWLEDGE_CONFIG_PATH")
	if path == "" {
		path = os.Getenv("DEN_KNOWLEDGE_CONFIG_PATH")
	}
	if path == "" {
