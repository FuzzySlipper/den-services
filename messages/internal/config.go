...
func LoadConfigFromPathWithValues(path string, values sharedconfig.Values) (*Config, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("reading messages config %s: %w", path, err)
	}
	var file configFile
	if err := yaml.Unmarshal(data, &file); err != nil {
		return nil, fmt.Errorf("parsing messages config %s: %w", path, err)
	}
...
func (c *Config) validate() error {
	...
	if strings.TrimSpace(c.TasksBaseURL) == "" {
		return errors.New("tasks base url is required")
	}
	if strings.TrimSpace(c.TasksToken) == "" {
		return errors.New("tasks token is required")
	}
...
func configPath() string {
	path := os.Getenv("MESSAGES_CONFIG_PATH")
	if path == "" {
