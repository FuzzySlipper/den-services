...
import (
	...
	projects "den-services/projects/internal"
...
func main() {
	if len(os.Args) == 2 && os.Args[1] == "--version" {
		fmt.Printf("projects %s %s %s\n", version, commit, builtAt)
		return
	}

	cfg, err := projects.LoadConfig()
	if err != nil {
	...
	defer pool.Close()
	service := projects.NewService(projects.NewStore(pool), time.Now)
	server, err := projects.NewHTTPServer(cfg, info, service)
	if err != nil {
		slog.Error("building server", "error", err)
		os.Exit(1)
	}
	slog.Info("projects listening", "bind_addr", cfg.BindAddr)
	if err := server.ListenAndServe(); err != nil {
		slog.Error("projects server", "error", err)
		os.Exit(1)
	}
}
...
func buildInfo() (health.BuildInfo, error) {
	...
	return health.NewBuildInfo("projects", version, commit, parsedBuiltAt)
}
