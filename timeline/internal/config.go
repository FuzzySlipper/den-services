...
type configFile struct {
	...
	Stream      streamConfigFile `yaml:"stream"`
...
func LoadConfigFromPath(path string) (*Config, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("reading timeline config %s: %w", path, err)
	}
	var file configFile
	if err := yaml.Unmarshal(data, &file); err != nil {
		return nil, fmt.Errorf("parsing timeline config %s: %w", path, err)
	}
	databaseURL, err := values.Expand(file.DatabaseURL)
	if err != nil {
		return nil, fmt.Errorf("expanding database_url: %w", err)
	}
	streamConfig, err := file.Stream.toConfig()
	if err != nil {
		return nil, err
	}
	...
	cfg := &Config{
		...
		ServiceToken: values.String("DEN_TIMELINE_SERVICE_TOKEN", ""),
...
func (c streamConfigFile) toConfig() (StreamConfig, error) {
	pollInterval, err := parseRequiredDuration("stream.poll_interval", c.PollInterval)
	if err != nil {
		return StreamConfig{}, err
	}
	heartbeatInterval, err := parseRequiredDuration("stream.heartbeat_interval", c.HeartbeatInterval)
	if err != nil {
		return StreamConfig{}, err
	}
	return StreamConfig{PollInterval: pollInterval, HeartbeatInterval: heartbeatInterval}, nil
}
...
func configPath() string {
	path := os.Getenv("TIMELINE_CONFIG_PATH")
	if path == "" {
